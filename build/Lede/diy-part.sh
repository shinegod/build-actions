...
# 后台IP设置
export Ipv4_ipaddr="10.10.31.10"            # 修改openwrt后台地址(填0为关闭)
...
# 更换固件内核
export Replace_Kernel="5.15.162"                    # 更换内核版本,在对应源码的[target/linux/架构]查看patches-x.x,看看x.x有啥就有啥内核了(填入内核x.x版本号,填0为不作修改)
